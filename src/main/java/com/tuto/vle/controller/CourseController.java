...
public class CourseController {
	...
	@ApiOperation(value = "View a list of available Courses", response = CourseDto.class, responseContainer = "List")
	...
	public List<CourseDto> getCoursesByUserId(
			...
			@ApiParam(value = "new/interest/subscription", required = false) @RequestParam(required = false) String filter)
			throws ResourceNotFoundException {
		return courseService.getCoursesByUserId(USER_ID, type, filter);
	}

	@ApiOperation(value = "Get course with course id", response = CourseDto.class, responseContainer = "List")
	@GetMapping("/courses/{id}")
	@ResponseStatus(HttpStatus.OK)
	public CourseDto getCourseDetailsByUniversityId(@PathVariable("id") int id) throws ResourceNotFoundException {
		return courseService.getCourseDetailsByCourseId(USER_ID, id);
	}
